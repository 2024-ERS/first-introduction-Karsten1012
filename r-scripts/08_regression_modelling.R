...
orchdat<-readr::read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vT6bdROUwchBBIreCQQdBenu35D5Heo8bl3UgES9wrpBwax_GUM1bKSo_QXfmLq8Ml9-crCI7MmW2XH/pub?gid=615066255&single=true&output=csv") |>
  dplyr::filter(year %in% c(2018,2019,2021:2024), Species_ID=="Orchestia_gammarellus", replicate<=3) |>
...
orchdat3|>
  ggplot(mapping=aes(x=factor(TransectPoint_ID),y=CountSum, group = year)) +
...
# y=b0 + b1x1 + b2*I(x^2) #expect b2 to be negative
